...
description: Convert the normalized SGA2 LaTeX tree (01-normalized_tex/) into JSON-with-embedded-HTML (02-converted_html/data/) plus paper.html, the entry page for the translation-viewer submodule (MathJax 3 SVG + XyJax-v3, vendored — offline, no CDN). The convert step of the pipeline; run after sga2-normalize.
---

# sga2-convert-html

Converts `01-normalized_tex/` → `02-converted_html/`: a JSON manifest plus
per-chapter content with embedded HTML under `data/`, plus `paper.html` — the
entry page for the `translation-viewer` submodule (mounted at the repo root),
a generic bilingual/aligned-column viewer. Math is kept as LaTeX in
`\(...\)` / `\[...\]` for client-side **MathJax 3 + XyJax-v3** (xy-pic
diagrams); it is not pre-rendered.
...
missing or older than the sources, (2) runs the converter, (3) generates
`paper.html` from `translation-viewer/paper.template.html`. Preview (must be
served from the repo root so `paper.html`'s `../translation-viewer/...`
references resolve):

```sh
python3 -m http.server   # then open http://localhost:8000/02-converted_html/paper.html
```
...
- **`\xymatrix` is passed through unchanged** — the XyJax-safety rewrites were
  already applied upstream by `sga2-inline-macros`; the viewer (translation-viewer)
  loads XyJax-v3.

## Output schema (`02-converted_html/`)

> Full, document-agnostic spec with field tables and machine-validatable JSON
> Schemas: [`docs/output-format.md`](../../../docs/output-format.md),
> `docs/manifest.schema.json`, `docs/chapter.schema.json`. The summary below is
> the quick reference.

```
data/config.json               project config: {baseLang, rightLangs, languages, ...}
data/fr/manifest.json          manifest (base language only): {toc, default_page_id, default_chapter_id, chapters, anchor_index}
data/fr/chapters/<id>.json     {chapter_id, title, number, pages[]}
data/en/chapters/<id>.json     same envelope; English content is empty stubs until translated (French is the reference)
paper.html                     entry page generated from translation-viewer/paper.template.html
```

`translation-viewer/` (a sibling submodule of `02-converted_html/`) supplies the
viewer engine and its own vendored MathJax 3 (SVG) + XyJax-v3 — nothing under
`02-converted_html/` needs to vendor those assets.

- **English non-clobber guard**: `emit()` writes the empty English stubs only while the
  English side is still untranslated. Once an `en/chapters/<id>.json` has
  non-empty pages (detected by `_en_chapter_is_stub`), a re-run **preserves**
  it instead of overwriting — so a filled-in English translation survives
  regenerating the French side. There is no English manifest — translation-viewer
  reads only the base-language manifest, for every column.
