...
BOOK_ROOT="$(cd "$SKILL_DIR/../../.." && pwd)"        # sga2/ — issues/ and 02-converted_html/ live here
...
# rooted at), e.g. "sga2/02-converted_html" — check.js needs this to build
...
if [ ! -f "$HTML_DIR/paper.html" ]; then
  echo "error: $HTML_DIR/paper.html does not exist — run sga2-convert-html first" >&2
  exit 2
fi
