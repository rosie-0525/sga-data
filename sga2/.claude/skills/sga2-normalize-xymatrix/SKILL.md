...
`rule_xymatrix_patches` in `.claude/skills/sga2-inline-macros/inline_macros.py`
...
`sga2-convert-html/convert.py` (`expand_math` now consumes `\\` atomically); it
...
## SGA2 occurrences fixed

All 11 diagrams plus the two diagrams discovered during the
verification pass. Final verified state.

| File:line | HTML | Category | Spelling adopted |
|---|---|---|---|
| chapter-04.tex:188 | IV-2 | (c) | `\ar@{^{(}->}[r]` |
| chapter-09.tex:11–16 | IX-1 | (d) + (f) + (e) | unwrap `array{c}`; rearrange `\ar[l] Y` → `Y \ar[l]`; primes → `^{\prime}` |
| chapter-10.tex:67–70 | X-2 | (b) | strip `\UseTips`/`\newdir` |
| chapter-10.tex:248–252 | X-3 | (b) + (e) | strip `\UseTips`/`\newdir`; primes → `^{\prime}` |
| chapter-11.tex:207 | XI-3 | (h) | `\Pic` → `{\mathrm{Pic}(…)}` (operator rewrite + cell-wrap) |
| chapter-12.tex:453–457 | XII-4(1) | (e) + (f) | primes → `^{\prime}`; rearrange `\ar[l]_-{…} U^{\prime}` → `U^{\prime}\ar[l]_-{…}` |
| chapter-12.tex:509–513 | XII-4(2) | (e) + (f) | primes → `^{\prime}`; rearrange `\ar[l]_-{…} U^{\prime}_0` → `U^{\prime}_0\ar[l]_-{…}` |
| chapter-14.tex:389–392 | XIV-1(1) | (a) + (g) + (j) | `{}\ar[r]` at row starts; wrap each `\underline{\mathrm{H}}…` cell-object |
| chapter-14.tex:395–398 | XIV-1(2) | (e) + (g) + (j) | primes → `^{\prime}`; wrap each script-bearing cell-object |
| chapter-14.tex:687–690 | XIV-2 | (a) + (e) + (h) | drop `{}` at row start; primes → `^{\prime}`; `\Spec k(s)` → `\mathrm{Spec}\,k(s)` |
| chapter-14.tex:805–808 | XIV-3(1) | (h) + (j) | `\Spec` → `\mathrm{Spec}\,`; wrap each `\mathrm{Spec}\,…` cell-object |
| chapter-14.tex:826–829 | XIV-3(2) | (a) + (e) + (i) | drop outer braces around `{X'}` etc.; primes → `^{\prime}` |
| chapter-14.tex:1034–1040 | XIV-3 | (d) | unwrap `equation*`+`array{c}` |

Added in the second verification pass (these were either never in the
original inventory or rendered as `mjx-merror` again under the current
XyJax-v3 + MathJax bundle, where the earlier (g)/(h) brace-dropping
proved insufficient and category (j) wrapping was needed):

| File:line | HTML | Category | Spelling adopted |
|---|---|---|---|
| chapter-04.tex:22–23 | IV-1 | (e) + (j) | prime → `^{\prime}`; wrap `{\ccat^{\circ}}`, `{\ccat^{\prime}}` |
| chapter-04.tex:216–218 | IV-3 | (j) | wrap `{\ccat^{\circ}_{Y}}`, `{\Ab\,.}`, `{\ccat_{Y}}` |
| chapter-14.tex:570–571 | XIV-1(3) | (j) | wrap each cell-object incl. `{\mathrm{R}^1j_*(…)}` |
| chapter-14.tex:629/659/745 | XIV-2 (X′ square, ×3) | (e) + (k) | primes → `^{\prime}`; `\\S'` kept via converter `\\`-fix |
| chapter-14.tex:689 | XIV-2 (Z′ square) | (e) + (h) + (k) | primes; `{\mathrm{Spec}\,k(s)}`; `\\S'_s` via converter `\\`-fix |

(The XIV-3 entry at lines 826–829 was not in the initial error
inventory because XIV-3's first failure at 805–808 was masking
subsequent diagrams. After the 805–808 fix, the parser progressed
far enough to surface the 826–829 issue.)
...
- [[sga2-inline-macros]] — hosts both `rule_xymatrix_safe` (regex rewrites
