#!/usr/bin/env python3
"""Convert hand-transcribed SGA 3 exposé HTML (01-transcribed/) into the JSON
...
cross-reference / footnote / back-arrow resolves.
...
# reads the base-language manifest; sga1/sga2 have none either). The empty
# stubs make the right pane show the standard "(translation not available)"
# placeholder — same behavior as sga1/sga2 for untranslated pages — until real
# translations replace them.
EN_DIR = OUT_DIR / "en"
EN_CH_DIR = EN_DIR / "chapters"

# Canonical reading order of the exposés. Files still in raw-pdftotext-draft
# form (no headings/theorems/math) are skipped and reported.
ORDER = ["avertissement", "introduction",
         "I", "II", "III", "IV", "V", "VIA", "VIB", "VIIA", "VIIB", "VIII",
...
FRONT_MATTER = {"avertissement", "introduction"}
...
def is_transcribed(src: str, roman: str = "") -> bool:
    ...
    Front matter (avertissement, introduction) is accepted with just an <h1>."""
...
def promote_stray_text(body):
    ...
    body.text), which the block walk below would silently drop (exposé VIA has
    two \\[\\xymatrix{…}\\] diagrams in that shape). Wrap each run in its own
    <p> so it becomes a block. (Ported from sga5/scripts/build_viewer_data.py.)"""
