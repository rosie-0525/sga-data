...
# Repeatedly ask Codex to transcribe the next small batch of SGA 7 pages.
...
RUN_TMP=$(mktemp -d "${TMPDIR:-/tmp}/sga7-transcribe.XXXXXX") || exit 1
...
while :; do
  ...
  PROMPT=$(printf '%s\n' \
    'Continue the manual transcription of SGA 7.' \
    '' \
    'First inspect sga7/00-original_pdf and sga7/01-transcribed to determine the current transcription status. Treat the existing 1.html as the formatting reference, detect partial output carefully, and do not duplicate pages that are already transcribed.' \
    '' \
    ...
    '' \
    "This is transcription run $iteration. You are explicitly authorized to send exactly one Slack message for this run. Before your final response, run /usr/bin/python3 sga7/slack_hook.py with a concise message containing the run number, the source PDF page range processed (or that transcription was already complete), the HTML output file, and the outcome. Do not send more than one message. If Slack delivery fails, mention the failure in your final response but still use the required final marker below." \
    '' \
    'If every PDF page in sga7/00-original_pdf was already faithfully transcribed before this run, or becomes fully transcribed and verified during this run, end your final response with a line containing exactly:' \
    'SGA7_TRANSCRIPTION_COMPLETE' \
    '' \
  ...
  elif [ "$(tail -n 1 "$LAST_MESSAGE")" = 'SGA7_TRANSCRIPTION_COMPLETE' ]; then
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] SGA 7 transcription is complete"
    exit 0
  else
