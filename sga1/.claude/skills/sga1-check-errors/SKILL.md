...
After `sga1-convert-html` has produced `02-converted_html/`. pdflatex accepts
...
```
bash .claude/skills/sga1-check-errors/check.sh          # both fr and en (default)
bash .claude/skills/sga1-check-errors/check.sh fr       # one language
bash .claude/skills/sga1-check-errors/check.sh fr,en    # explicit
```
...
two-language tree (~100 pages × 2 + a titles pass each) takes roughly 1–2 min.
...
step** (`sga1-inline-macros` / the converter), not in HTML post-processing — see
...
- `.claude/skills/sga1-convert-html/` — produces the viewer this skill checks.
