...
BOOK_ROOT="$(cd "$SKILL_DIR/../../.." && pwd)"        # sga1/ — issues/ and 02-converted_html/ live here
SUPER_ROOT="$(cd "$SKILL_DIR/../../../.." && pwd)"    # the sga/ super-repo root — translation-viewer/ lives here
HTML_DIR="$BOOK_ROOT/02-converted_html"
OUT_DIR="$BOOK_ROOT/issues"
# HTML_DIR's path relative to SUPER_ROOT (the dir the http server below is
# rooted at), e.g. "sga1/02-converted_html" — check.js needs this to build
# paper.html's URL, since the server root sits one level above every book.
...
if [ ! -f "$HTML_DIR/paper.html" ]; then
  echo "error: $HTML_DIR/paper.html does not exist — run sga1-convert-html first" >&2
  exit 2
fi

if [ ! -f "$SUPER_ROOT/translation-viewer/viewer.js" ]; then
  echo "error: $SUPER_ROOT/translation-viewer/viewer.js does not exist — translation-viewer/ is a plain shared folder at the super-repo root; check it is present there" >&2
  exit 2
fi
...
node "$SKILL_DIR/check.js" "$HTML_DIR" "http://localhost:$PORT" "$OUT_DIR" "$LANGS" "$HTML_DIR_REL"
rc=$?
