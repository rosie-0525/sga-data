// Headless render/error scan for the SGA1 deliverable.
//
...
// the shared translation-viewer engine, NOT a tree of standalone HTML files:
//   paper.html                — bootstraps translation-viewer/viewer-bootstrap.js,
//                                which fetches data/config.json, configures MathJax 3
//                                (SVG) + XyJax-v3 from it, and injects its own
//                                vendored assets (offline, no CDN)
...
// "sga1/02-converted_html"). Falls back to the bare basename for direct/manual
...
// the <div class="equation"> id and the rest as label-anchor spans — nested
// inside the div (current convert.py; matches translation-viewer's blockEl(),
// which keeps only the first top-level element of a block's html) or, for any
// legacy content, as preceding sibling spans. Both shapes are scanned here so a
// leftover pre-migration block still gets checked correctly. Each numbered row
// should display a number via an explicit \tag{} or \tag*{} (MathJax runs
// tags:'none', so only \tag produces a number, and multi-row envs are forced to
// their starred form). If a block carries more eq: labels than \tag{}s, a
// labeled row renders with NO number — e.g. (21)/(21 bis) where the (21) row
// silently loses its tag. We report the shortfall by block; \notag/\nonumber in
// the body is recorded so a legitimately-unnumbered row isn't mistaken for the
// bug. Fix belongs upstream in the converter (per the README).
function staticTagIntegrity(html) {
  // Optional preceding label-anchor spans (legacy shape), then the equation div.
  ...
  while ((m = re.exec(html)) !== null) {
    ...
    // count both \tag{..} (numbered) and \tag*{..} (custom label, e.g. "a)") —
    // a \tag* still produces a visible number/label, so it is not a dropout.
    const tagCount = (body.match(/\\tag\s*\*?\s*\{/g) || []).length;
...
function summarise(results) {
  ...
  if (xrefBad.length) {
    ...
    if (refLabelIndex.size > 0) {
      lines.push('');
      lines.push('Unresolved refs in math by label (fix upstream in sga1-inline-macros):');
