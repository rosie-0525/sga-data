#!/usr/bin/env python3
"""
sga1-convert-html: convert the normalized SGA1 LaTeX tree
(01-normalized_tex/) into JSON-with-embedded-HTML (02-converted_html/).

Pure Python standard library. Math is kept as LaTeX in \\(...\\) / \\[...\\]
for client-side MathJax 3 + XyJax-v3; the content macros from
sga1-macros.sty are expanded to plain LaTeX. Numbering and cross-references are
...
# --------------------------------------------------------------------------
# Section 2. Math macro expansion (from sga1-macros.sty)
# --------------------------------------------------------------------------

# 0-argument math macros: name -> replacement (standard LaTeX/MathJax).
# Mirrors the content macros in sga1-macros.sty; keep the two in sync.
MATH_0ARG = {}
MATH_0ARG.update({
    # blackboard-bold / bold letters
    'AA': r'\mathbf{A}', 'NN': r'\mathbf{N}', 'CC': r'\mathbf{C}', 'GG': r'\mathbf{G}',
    'LL': r'\mathbb{L}', 'PP': r'\mathbf{P}', 'QQ': r'\mathbf{Q}', 'RR': r'\mathbf{R}',
    'VV': r'\mathbf{V}', 'ZZ': r'\mathbf{Z}', 'WW': r'\mathbf{W}',
    # bold "category" names
    'Cat': r'\mathbf{Cat}', 'Ens': r'\mathbf{Ens}', 'Sch': r'\mathbf{Sch}',
    'Univ': r'\mathbf{Univ}',
    # roman shorthands
    'an': r'\mathrm{an}', 'cart': r'\mathrm{cart}', 'Fib': r'\mathrm{Fib}',
    'et': r'\mathrm{\acute{e}t}', 'red': r'\mathrm{r\acute{e}d}',
    'res': r'\mathrm{r\acute{e}s}', 'alg': r'\mathrm{alg}', 'pr': r'\mathrm{pr}',
    'tame': r'\mathrm{t}', 'sp': r'\mathrm{sp}', 'id': r'\mathrm{id}',
    'H': r'\mathrm{H}', 'R': r'\mathrm{R}', 'kres': r'\mathit{k}',
    'rd': r'd', 'tH': r'{}^t\!H',
    # operators (DeclareMathOperator -> \operatorname)
    'Aut': r'\operatorname{Aut}', 'SheafAut': r'\operatorname{\mathbf{Aut}}',
    'car': r'\operatorname{car}', 'card': r'\operatorname{card}',
    'codim': r'\operatorname{codim}', 'Coker': r'\operatorname{Coker}',
    'coprof': r'\operatorname{coprof}', 'diag': r'\operatorname{diag}',
    'divisor': r'\operatorname{div}', 'Ext': r'\operatorname{Ext}',
    'Fer': r'\operatorname{Fer}', 'Fl': r'\operatorname{Fl}',
    'CatFl': r'\operatorname{\mathbf{Fl}}', 'Gl': r'\operatorname{\mathbf{GL}}',
    'GL': r'\operatorname{\mathbf{GL}}', 'gr': r'\operatorname{gr}',
    'Gal': r'\operatorname{Gal}', 'Hom': r'\operatorname{Hom}',
    'SheafHom': r'\operatorname{\mathbf{Hom}}', 'hop': r'\operatorname{hop}',
    'Imm': r'\operatorname{Im}', 'Im': r'\operatorname{Im}',
    'Isom': r'\operatorname{Isom}', 'SheafIsom': r'\operatorname{\mathbf{Isom}}',
    'Ker': r'\operatorname{Ker}', 'Lim': r'\operatorname*{Lim}',
    'Norm': r'\operatorname{N}', 'Ob': r'\operatorname{Ob}',
    'Ouv': r'\operatorname{Ouv}', 'Pic': r'\operatorname{Pic}',
    'Pro': r'\operatorname{Pro}', 'Prodash': r'\operatorname{Pro\text{-}}',
    'prof': r'\operatorname{prof}', 'Proj': r'\operatorname{Proj}',
    'Quot': r'\operatorname{Quot}', 'Spec': r'\operatorname{Spec}',
    'Supp': r'\operatorname{Supp}', 'Symm': r'\operatorname{Symm}',
    'Symp': r'\operatorname{\mathbf{Sp}}', 'Tor': r'\operatorname{Tor}',
    'trace': r'\operatorname{tr}', 'Ind': r'\operatorname{Ind}',
    # composite / misc
    'isomto': r'\overset{\sim}{\longrightarrow}',
    'isomfrom': r'\overset{\sim}{\longleftarrow}',
    'bbullet': r'{\scriptscriptstyle\bullet}',
    'bbalpha': r'\boldsymbol{\alpha}', 'bbmu': r'\boldsymbol{\mu}',
    'quoi': '', 'steco': '',
    'tbigcap': r'\mathop{\textstyle\bigcap}\limits',
    'tbigwedge': r'\mathop{\textstyle\bigwedge}\limits',
    'varinjLim': r'\varinjlim', 'varprojLim': r'\varprojlim',
    # arrows / relations
    ...
    'dpl': r'\displaystyle',
    # text/citation macros that may appear inside math, usually inside a \text{}
    # (prose in display math). Expand to PLAIN text: \mathrm/\text wrappers both
    # break — \mathrm is illegal in the surrounding text mode, and a nested
    # \text is illegal inside \text. Plain letters render upright in \text and
    # (rarely) as math italics in bare math, which is acceptable.
    'EGA': 'EGA ', 'SGA': 'SGA ',
    'S': '§', 'oldS': '§',
    'cf': 'cf. ', 'Cf': 'Cf. ', 'ie': 'i.e. ', 'iev': 'i.e., ',
    'resp': 'resp. ', 'loccit': 'loc. cit. ', 'numero': 'nº ',
    'No': 'Nº ', 'no': 'nº ',
})
...
MATH_1ARG = {
    ...
    'mlto': r'\overset{#1}{\longmapsto}',
    'mathcal': r'\mathscr{#1}',
    'cal': r'\mathscr{#1}',
    'othercal': r'\mathscr{#1}',
    'Bbb': r'\mathbb{#1}',
    'rm': r'\mathrm{#1}', 'it': r'\mathit{#1}', 'bf': r'\mathbf{#1}',
    'sf': r'\mathsf{#1}', 'tt': r'\mathtt{#1}',
    'bar': r'\overline{#1}', 'hat': r'\widehat{#1}', 'tilde': r'\widetilde{#1}',
}

# n-argument math macros (incl. chapter-local \newcommand): name -> (nargs, template)
MATH_NARG = {
    'leftexp': (2, r'{\vphantom{#2}}^{#1}{#2}'),
}

# (text/citation math-safe macros are folded into MATH_0ARG above)
...
def expand_math(s):
    """Expand sga1-macros.sty math macros to plain LaTeX, to a fixpoint."""
    for _ in range(10):
        out = []
        i = 0
        n = len(s)
...
DROP_WITH_ARGS = {
    ...
    'value': (0, 1), 'marginpar': (0, 1), 'oldindexnot': (0, 1),
}
...
THEOREM_CAPTIONS = {
    ...
    'proposition': 'Proposition', 'propositionstar': 'Proposition',
    'subproposition': 'Proposition',
    'lemme': 'Lemme', 'sublemme': 'Lemme',
    'corollaire': 'Corollaire', 'corollaires': 'Corollaires',
    'corollairestar': 'Corollaire',
    'theoremedefinition': 'Théorème et définition',
    'theoremedepurete': 'Théorème de pureté',
    'definition': 'Définition', 'definitions': 'Définitions',
    'definitionstar': 'Définition', 'subdefinition': 'Définition',
    'remarque': 'Remarque', 'remarques': 'Remarques',
    'remarquestar': 'Remarque', 'remarquesstar': 'Remarques',
    'subremarque': 'Remarque', 'remarqueMR': 'Remarque',
    'exemple': 'Exemple', 'exemples': 'Exemples',
    'scholiestar': 'Scholie',
}
THEOREM_REMARK_STYLE = {'definition', 'definitions', 'definitionstar', 'subdefinition',
                        'remarque', 'remarques', 'remarquestar', 'remarquesstar',
                        'subremarque', 'remarqueMR', 'exemple', 'exemples', 'scholiestar'}
LIST_ENVS = {'enumerate': 'ol', 'enumeratei': 'ol', 'enumeratea': 'ol',
             'enumerateb': 'ol', 'itemize': 'ul', 'theindex': 'ul'}
...
_COUNTER_MACRO = re.compile(
    r'\\(?:the(?:chapter|section|subsection|subsubsection|paragraph)'
    r'|arabic|roman|Roman|alph|Alph)(?![a-zA-Z])')


def _resolve_counter_tags(body, ctx):
    r"""Replace \tag{...}/\tag*{...} whose content has an unevaluable LaTeX
    counter macro (\thesubsubsection, \arabic{...}, ...) with the authoritative
    number from main.aux (via a \label in the block); MathJax cannot evaluate
    counters, so the raw macro would otherwise leak as text. Falls back to
    stripping the counter macros if no aux number is available."""
    if ctx is None or '\\tag' not in body:
        return body
    aux_num = None
    for k in re.findall(r'\\label\{([^}]*)\}', body):
        info = ctx.labels.get(k)
        if info and info.get('display'):
            aux_num = info['display'].replace('$', '')
            break
    out = []
    i = 0
    n = len(body)
    pat = re.compile(r'\\tag(\*?)\s*\{')
    while i < n:
        m = pat.search(body, i)
        if not m:
            out.append(body[i:])
        ...
        inner, after = _read_group(body, m.end() - 1)
        if _COUNTER_MACRO.search(inner):
            if aux_num:
                inner = aux_num
            else:
                inner = _COUNTER_MACRO.sub('', inner)
                inner = re.sub(r'\{[A-Za-z]+\}', '', inner)
        out.append('\\tag%s{%s}' % (m.group(1), inner))
        i = after
    return ''.join(out)
...
def render_mathblock(kind, body, ctx):
    ...
    body = _resolve_counter_tags(body, ctx)
    ...
    # element — the shared translation-viewer engine's blockEl() keeps only
    # the first top-level element of a block's HTML, so a preceding sibling
    # span would silently drop the div carrying the actual math.
    extra = ''
    if len(labels) > 1:
        extra = ''.join(
...
def render_list(name, inner, ctx):
    ...
    cls = {'enumerate': 'enumerate', 'enumeratei': 'enumerate-i',
           'enumeratea': 'enumerate-a', 'enumerateb': 'enumerate',
           'itemize': 'itemize', 'theindex': 'index'}[name]
...
SLUG_MAP = {
    'préface': 'preface', 'preface': 'preface',
    'introduction': 'introduction', 'avertissement': 'avertissement',
    'résumé': 'resume', 'resume': 'resume',
    'index des notations': 'index-notations',
    'index terminologique': 'index-terminologie',
...
def write_config(out_dir):
    ...
    user-edited content, so it is written unconditionally on every run. Strings
    below are carried over verbatim from the pre-migration self-contained
    viewer (viewer/index.html, viewer/viewer.js) so the migration to the shared
    translation-viewer engine doesn't change any user-facing text."""
    config = {
        'baseLang': 'fr',
        'rightLangs': ['en'],
        'languages': {
            'fr': {
                'label': 'FR',
                'pageTitle': 'SGA 1 — Revêtements étales et groupe fondamental',
                'bookTitle': 'SGA 1',
...
def file_specs(src):
    ...
    if os.path.exists(fm):
        specs.append((fm, ('preface', 'Préface')))
    for p in sorted(glob.glob(os.path.join(src, 'chapter-*.tex'))):
        stem = os.path.splitext(os.path.basename(p))[0]
        specs.append((p, (stem, 'Exposé')))
    bm = os.path.join(src, 'back-matter.tex')
    if os.path.exists(bm):
        specs.append((bm, ('index', 'Index des notations')))
    return specs
