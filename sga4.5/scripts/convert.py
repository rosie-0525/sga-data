#!/usr/bin/env python3
"""
sga4.5-convert-html: convert the SGA 4 1/2 LaTeX source tree
(sga4.5/) into JSON-with-embedded-HTML (02-converted_html/).

Pure Python standard library. Math is kept as LaTeX in \\(...\\) / \\[...\\]
for client-side MathJax 3 + XyJax-v3; content macros from sga-style.sty are
expanded to plain LaTeX. Numbering and cross-references are
resolved authoritatively from the compiled sga4.5.aux.

Output (per the project schema, with clean readable ids; consumed by the
translation-viewer submodule via 02-converted_html/paper.html):
  02-converted_html/data/config.json          project config {baseLang, rightLangs, languages}
  02-converted_html/data/fr/manifest.json      manifest {toc, chapters, default_*} (base language only)
  02-converted_html/data/fr/chapters/<id>.json {chapter_id, title, pages[]}
  02-converted_html/data/en/chapters/<id>.json translation content (empty stubs until filled in)
...
# --------------------------------------------------------------------------
# Section 1. aux parser -> labels, toc, bibcites
# --------------------------------------------------------------------------
...
def parse_aux(aux_path):
    ...
    def read_aux_tree(path):
        ...
        children = []
        for match in re.finditer(r'\\@input\{([^}]*)\}', value):
            children.append(read_aux_tree(os.path.join(os.path.dirname(path),
                                                       match.group(1))))
        return value + '\n' + '\n'.join(children)

    text = read_aux_tree(aux_path)
...
# --------------------------------------------------------------------------
# Section 2. Math macro expansion
# --------------------------------------------------------------------------
...
# SGA 4 1/2's sga-style.sty creates these families programmatically.
for _c in 'ABCDEFGHIJKLMNOPQRSTUVWXYZ':
    MATH_0ARG['c' + _c] = r'\mathcal{%s}' % _c
    MATH_0ARG['d' + _c] = r'\mathbb{%s}' % _c
    MATH_0ARG['f' + _c] = r'\mathfrak{%s}' % _c
    MATH_0ARG['e' + _c] = r'\mathsf{%s}' % _c
    MATH_0ARG['s' + _c] = r'\mathscr{%s}' % _c
for _c in 'abcdefghjklmnopqrstuvwxyz':
    MATH_0ARG['f' + _c] = r'\mathfrak{%s}' % _c
MATH_0ARG.update({
    'aut': r'\operatorname{Aut}', 'br': r'\operatorname{Br}',
    'cl': r'\operatorname{cl}', 'cosp': r'\operatorname{cosp}',
    'dv': r'\operatorname{div}', 'Div': r'\operatorname{Div}',
    'ext': r'\operatorname{Ext}', 'gal': r'\operatorname{Gal}',
    'hh': r'\mathsf{H}', 'im': r'\operatorname{im}',
    'ob': r'\operatorname{Ob}', 'pgl': r'\operatorname{PGL}',
    'pic': r'\operatorname{Pic}', 'res': r'\operatorname{R}',
    'sh': r'\operatorname{Sh}', 'spe': r'\operatorname{sp}',
    'spec': r'\operatorname{Spec}', 'swan': r'\operatorname{Sw}',
    'tor': r'\operatorname{Tor}', 'Tor': r'\underline{\operatorname{Tor}}',
    'rhom': r'\mathsf{R}\operatorname{Hom}',
    'rHom': r'\mathsf{R}\underline{\operatorname{Hom}}',
    'lotimes': r'\overset{\mathsf{L}}{\otimes}',
    'iso': r'\xrightarrow{\sim}', 'kloos': r'\mathrm{K}',
    'pr': r'\mathrm{pr}', 'dmu': r'\boldsymbol{\mu}',
    'bigboxtimes': r'\boxtimes',
})
...
def expand_math(s):
    """Expand source math macros to plain LaTeX, to a fixpoint."""
    for _ in range(10):
        out = []
        i = 0
        n = len(s)
...
def math_html(tex, ctx=None):
    ...
    # Lift nested $...$ fragments out of text-mode commands while preserving
    # the prose on either side: \text{for $i>0$} -> \text{for } i>0.
    s = _lift_nested_math_from_text(tex)
    # Standard one-argument accents are occasionally used without braces
    # before a source macro (e.g. \widehat\dZ). Bracing lets the next pass
    # expand that inner macro without making the outer command idempotently
    # swallow it on every fixpoint iteration.
    s = re.sub(r'\\(widehat|widetilde|overline|underline|xrightarrow|xleftarrow)\s*(\\[A-Za-z]+)',
               r'\\\1{\2}', s)
    s = expand_math(s)
    s = re.sub(r'\\tag\{\\ast([^}]*)\}',
               lambda m: r'\tag{*' + m.group(1).replace('_', '') + '}', s)
    s = _normalize_xymatrices(s)
    if ctx is not None:
        def _num(key):
...
def _lift_nested_math_from_text(s):
    names = ('text', 'textnormal', 'textrm', 'mbox')
    out = []
    i = 0
    while i < len(s):
        match = re.match(r'\\(' + '|'.join(names) + r')\s*\{', s[i:])
        if not match:
            out.append(s[i])
            i += 1
            continue
        name = match.group(1)
        brace = i + match.end() - 1
        inner, after = _read_group(s, brace)
        if '$' not in inner:
        ...
        for index, part in enumerate(parts):
            if not part:
                continue
            rendered.append(('\\%s{%s}' % (name, part)) if index % 2 == 0 else part)
        out.append(' '.join(rendered))
        i = after
    return ''.join(out)


def _normalize_xymatrices(s):
    """Move leading reverse arrows behind their XyJax cell object.

    Classic xy-pic accepts ``& \\ar[l]_-f Y``; XyJax-v3 expects the object
    first (``& Y \\ar[l]_-f``). The transformation is limited to top-level
    cells inside xymatrix groups.
    """
    out = []
    i = 0
    while i < len(s):
        match = re.search(r'\\xymatrix(?:@[^\{\s]*)?\s*\{', s[i:])
        if not match:
            out.append(s[i:])
            break
        start = i + match.start()
        brace = i + match.end() - 1
        inner, after = _read_group(s, brace)
        out.append(s[i:brace + 1])
        out.append(_normalize_xy_cells(inner))
        out.append('}')
        i = after
    return ''.join(out)


def _normalize_xy_cells(body):
    body = re.sub(r'\\(?:mbox|text)\{([.,;:])\}', r'\1', body)
    body = re.sub(r'([_^])(-?)(\\[A-Za-z]+\{[^{}]*\})', r'\1\2{\3}', body)
    body = re.sub(r'\{(\\mathscr\{[^{}]*\}\x27)\}(?=\^)', r'\1', body)
    body = re.sub(r'\{([A-Za-z][A-Za-z0-9_]*)\}_(?=\\ast)', r'(\1)_', body)
    pieces = []
    current = []
    depth = 0
    i = 0
    while i < len(body):
        if body[i] == '{':
            depth += 1
        elif body[i] == '}' and depth:
            depth -= 1
        if depth == 0 and body[i] == '&':
            pieces.append(_normalize_xy_cell(''.join(current)))
            pieces.append('&')
            current = []
            i += 1
            continue
...
def _normalize_xy_cell(cell):
    leading = len(cell) - len(cell.lstrip())
    pos = leading
    if not cell.startswith('\\ar', pos):
        return cell
    arrow_start = pos
    while cell.startswith('\\ar', pos):
        pos += 3
        while pos < len(cell):
            if cell[pos].isspace():
                pos += 1
            ...
            if cell[pos] == '@':
                pos += 1
                if pos < len(cell) and cell[pos] == '{':
                    _, pos = _read_group(cell, pos)
                elif pos < len(cell) and cell[pos] == '<':
                    end = cell.find('>', pos + 1)
                    pos = len(cell) if end < 0 else end + 1
                else:
                    while pos < len(cell) and cell[pos] not in '[ _^\\\n':
                        pos += 1
                continue
            if cell[pos] == '[':
                _, pos = _read_optional(cell, pos)
                continue
            if cell[pos] in '_^':
                pos += 1
                while pos < len(cell) and cell[pos] in '-+|':
                    pos += 1
                if pos < len(cell) and cell[pos] == '{':
                    _, pos = _read_group(cell, pos)
                elif pos < len(cell) and cell[pos] == '\\':
                    token = re.match(r'\\(?:[A-Za-z]+|.)', cell[pos:])
                    pos += token.end() if token else 1
                elif pos < len(cell):
                    pos += 1
                continue
            break
        while pos < len(cell) and cell[pos].isspace():
            pos += 1
        if not cell.startswith('\\ar', pos):
            break
    arrows = cell[arrow_start:pos].strip()
    content = cell[pos:].strip()
    if not content:
        return cell
    return cell[:leading] + content + ' ' + arrows
...
def ref_link(ctx, key, kind):
    ...
    if re.search(r'[\\_^]', num):
        safe = '\\(' + math_html(num, ctx) + '\\)'
    else:
    ...
    while i < n:
        ...
        if c == '$':
            ...
            while j < n:
                if s[j] == '{' and s[j - 1] != '\\':
                    brace_depth += 1
                elif s[j] == '}' and s[j - 1] != '\\' and brace_depth:
                    brace_depth -= 1
                elif s[j] == '$' and s[j - 1] != '\\' and brace_depth == 0:
                    break
                j += 1
            body = s[i + 1:j]
...
THEOREM_CAPTIONS = {
    ...
    'theorem': 'Théorème', 'theorem_': 'Théorème', 'theorem*': 'Théorème',
    'lemma': 'Lemme', 'lemma_': 'Lemme',
    'corollary': 'Corollaire', 'corollary_': 'Corollaire',
    'proposition_': 'Proposition', 'definition_': 'Définition',
    'prop-def_': 'Proposition-définition',
}
...
def register_and_strip_defs(text):
    ...
    n = len(text)
    while i < n:
        m = re.match(r'\\(?:newcommand|renewcommand|providecommand)', text[i:])
        if m:
...
def render_mathblock(kind, body, ctx):
    ...
    # them resolve instead of dangling. These live *inside* the block's div
    # (not as preceding sibling elements) so the block stays a single root
    # element — viewers that align content block-by-block (e.g. taking only
    # the first top-level element of a block's HTML) would otherwise silently
    # drop the div carrying the actual math.
    extra = ''
    if len(labels) > 1:
        extra = ''.join(
    ...
    html_block = '<div class="%s" id="%s">\n%s%s\n</div>' % (
        cls, html.escape(bid, quote=True), extra, tex)
    ...
    for path, orphan in file_specs:
        ...
        for ev in iter_top(text):
            ...
            if kind == 'sec' and ev[1] == 'chapter':
                ...
                if cid is None:
                    cid = (orphan[0] if not orphan[0].startswith('expose-')
                           else slugify(_plain_text(short_title)))
...
def write_config(out_dir):
    """data/config.json — read by translation-viewer/viewer-bootstrap.js. No
    user-edited content, so it is written unconditionally on every run. The
    default mathjax packages ['xypic', 'ams', 'color', 'mathtools'] and no
    macros are what this project needs, so the 'mathjax' key is omitted."""
    config = {
        'baseLang': 'fr',
        'rightLangs': ['en'],
        'languages': {
            'fr': {
                'label': 'FR',
                'pageTitle': 'SGA 4½ — Cohomologie étale',
                'bookTitle': 'SGA 4½',
                'toc': 'Table des matières',
                'notrans': '(pas de traduction)',
            ...
            'en': {
                'label': 'EN',
                'pageTitle': 'SGA 4½ — Étale Cohomology',
                'bookTitle': 'SGA 4½',
                'toc': 'Table of contents',
                'notrans': '(no translation)',
    ...
    with open(os.path.join(out_dir, 'data', 'config.json'), 'w', encoding='utf-8') as f:
        json.dump(config, f, ensure_ascii=False, indent=1)


def emit(chapters, out_dir):
    wire_citations(chapters)
    fr_dir = os.path.join(out_dir, 'data', 'fr', 'chapters')
    en_dir = os.path.join(out_dir, 'data', 'en', 'chapters')
    ...
    for ch in chapters:
        ...
        for pi, page in enumerate(ch.pages):
            ...
            anchor_index.setdefault(page.id, page.id)
            for blk in page.blocks:
                if blk.get('id'):
    ...
    # Base-language manifest only — translation-viewer reads this one manifest
    # to drive navigation for every language column; it is always regenerated.
    # fr_dir's makedirs above already ensures data/fr/ exists.
    with open(os.path.join(out_dir, 'data', 'fr', 'manifest.json'), 'w', encoding='utf-8') as f:
...
def file_specs(src, bbl=None):
    names = [
        ('sga4.5-note.tex', ('note', 'Note du typographe')),
        ('sga4.5-intro.tex', ('introduction', 'Introduction')),
        *[('sga4.5-%d.tex' % k, ('expose-%d' % k, 'Exposé %d' % k))
          for k in range(1, 9)],
        ('sga4.5-erratum.tex', ('erratum', 'Erratum pour SGA 4, tome 3')),
    ]
    specs = [(os.path.join(src, name), orphan) for name, orphan in names
             if os.path.exists(os.path.join(src, name))]
    if bbl and os.path.exists(bbl):
        specs.append((bbl, ('bibliographie', 'Bibliographie')))
    return specs


def run(src, out, only=None, aux=None, bbl=None):
    aux = aux or os.path.join(src, 'sga4.5.aux')
...
def verify(chapters, ctx, out):
    ...
    for c in chapters:
        for p in c.pages:
            for b in p.blocks:
                if b['type'] in ('equation', 'displaymath'):
                    continue
                h = b['html']
...
def main():
    ...
    book_root = os.path.abspath(os.path.join(here, '..'))
    ap.add_argument('--src', default=os.path.join(book_root, 'sga4.5'))
    ap.add_argument('--out', default=os.path.join(book_root, '02-converted_html'))
    ap.add_argument('--aux', default=None)
    ap.add_argument('--bbl', default=None)
    ap.add_argument('--only', default=None)
    ap.add_argument('--verify', action='store_true')
    args = ap.parse_args()
    chapters, ctx, manifest = run(args.src, args.out, args.only, args.aux, args.bbl)
