#!/usr/bin/env python3
"""Convert hand-transcribed SGA 6 exposé HTML (01-transcribed/) into the JSON
...
pages split at <h2> boundaries), with the SGA 6 transcription quirks handled:

  * Exposé 0 has a trailing SGA-4-style <div id="footnotes"> list with
    <li id="fnN"> items, in-text markers <a id="fnrefN" href="#fnN"> — the
    sga5 list handling covers it (markers found by href, ids prefixed
    "<roman>-fn…" so every id is globally unique in the viewer's single
    anchor_index);
  * all other footnotes are *inline*: <p class="footnote"> (markerless, the
    printed "(1)"/"(*)" mark leads the text, usually inside <sup>) or
    <aside class="footnote" id="fn-XIII-1"> (linked from an in-text
    <a href="#fn-XIII-1">) sitting in the block stream at their printed page
    position. They are lifted out into the page's footnotes list; markerless
    ones get synthetic ids and keep their printed mark as the number;
  * Exposé XIV has no theorem environments, so a finished transcription is
    recognized by its <h2 id=…> sections alone;
  * a secondary <h1> would start a new page like an <h2> (none in SGA 6).
...
# Canonical reading order: the exposés of the published SGA 6 volume
# (Lecture Notes 225) — there is no Exposé XI.
...
def is_transcribed(src: str, roman: str = "") -> bool:
    ...
    draft is just <p> dumps with bare page-number paragraphs. (XIV has no
    theorem environments, so unlike sga5 we don't require class="thm".)"""
...
def convert(roman: str, src: str):
    ...
    def is_fn_list(el):
        # 0.html has two lists: <div id="footnotes"> (programme, fn1-2) and
        # <div id="app-footnotes"> (appendix, fn3-17)
        classes = (el.get("class") or "").split()
        return el.tag == "div" and ("footnotes" in classes
                                    or (el.get("id") or "").endswith("footnotes"))

    def is_inline_fn(el):
        classes = (el.get("class") or "").split()
        return el.tag in ("p", "aside") and "footnote" in classes

    # printed footnote mark leading a markerless inline footnote's text,
    # e.g. "<sup>(1)</sup> …", "<sup>(*)</sup> …", "(∗) …"
    MARK_RE = re.compile(r"^(?:<sup>)?\((\*+|∗+|¹|²|³|\d+)\)(?:</sup>)?\s*")

    def take_inline_fn(el):
        """Lift one <p class=footnote>/<aside class=footnote> into footnotes."""
        nonlocal fn_seen
        fn_seen += 1
        item_id = el.get("id") or ""
        if not FN_ID.match(item_id):
            item_id = f"fn-x{fn_seen}"
        html = inner_html(el)
        if el.tag == "aside":      # unwrap the single <p> the aside holds
            inner = LH.fromstring(f"<div>{html}</div>")
            if len(inner) == 1 and inner[0].tag == "p" and not (
                    inner.text or "").strip():
                html = inner_html(inner[0])
        printed_mark = ""
        m = MARK_RE.match(html)
        if m:
    ...
    for i, el in enumerate(children):
        if is_inline_fn(el):
            take_inline_fn(el)
            continue
        # a footnote nested inside a block (rare): lift it out before the
        # block is serialized
        for sub in el.xpath('.//p[contains(@class,"footnote")]'
                            ' | .//aside[contains(@class,"footnote")]'):
            take_inline_fn(sub)
            sub.drop_tree()
        if is_fn_list(el):
